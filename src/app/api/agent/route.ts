...
- Add a short in-text citation immediately after the supported claim. Use the source title and its most useful locator when available, for example "(HR Policy, p. 35)", "(Finance Policy, clause 4.2)", or "(Erth Zayed AI Avatar Knowledge Base, FAQ 02)". Omit the locator only when the source provides none.
...
- Do not end with generic offers such as "If you want, I can..." unless a necessary clarification is required.`;
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        const retrievedSourceFilenames = new Set<string>();
        let isAborted = false;
        ...
        const emitDisplayContent = (text: string, separator = '') => {
          const displayText = preserveTechnicalFilenames ? text : formatTextForDisplay(text, [...retrievedSourceFilenames]);
        ...
        try {
          ...
          for await (const rawEvent of events) {
            ...
            if (event.event === 'on_tool_end' && event.name === 'knowledge_base') {
              ...
              sources.forEach((source) => retrievedSourceFilenames.add(source.filename));
              controller.enqueue(new TextEncoder().encode(`data: ${JSON.stringify({
                event: 'sources',
