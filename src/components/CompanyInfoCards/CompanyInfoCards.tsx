...
export const CompanyInfoCards: FC<CompanyInfoCardsProps> = ({ entity, isVisible, theme = 'dark' }) => {
  ...
  return (
    <motion.div
      className={`absolute flex flex-col pointer-events-auto z-60 ${isPortrait
          ? 'gap-2 w-80 right-4' + (!showCompanyInfo && entity.coordinates ? ' top-1/2 -translate-y-1/2' : ' bottom-40')
          : 'gap-4 w-full max-w-md lg:max-w-lg xl:max-w-xl right-12' + (!showCompanyInfo && entity.coordinates ? ' top-1/2 -translate-y-1/2' : ' top-32')
        }`}
        ...
        {isVisible && (
          <>
            ...
            {!showCompanyInfo && entity.coordinates && (
              <motion.div
                key="location-only"
                className={`w-full border backdrop-blur-xl shadow-lg rounded-lg overflow-hidden ${cardBg} ${isPortrait ? 'p-2' : 'p-4'
                  }`}
            ...
            {showCompanyInfo && (
              <>
                ...
                <motion.div
                  key="company-header"
                  className={`w-full border backdrop-blur-xl shadow-lg rounded-lg ${cardBg} ${isPortrait ? 'p-2' : 'p-4'
                    }`}
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: 20 }}
                ...
                >
                  <h3 className={`${headingColor} font-light tracking-tight ${isPortrait ? 'text-sm mb-0.5' : 'text-lg mb-1'
                    }`}>
                    {entity.name}
                  </h3>
                  <p className={`${subTextColor} font-light ${isPortrait ? 'text-xs mb-1.5' : 'text-sm mb-2'
                    }`}>{entity.type}</p>
                ...
                {metricPills.length > 0 && (
                  <motion.div
                    key="metrics"
                    className={`w-full flex flex-wrap border backdrop-blur-xl shadow-lg rounded-lg ${cardBg} ${isPortrait ? 'gap-1.5 p-2' : 'gap-2 p-4'
                      }`}
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: 20 }}
                  ...
                  >
                    {metricPills.map((pill) => (
                      <div
                        key={pill.label}
                        className={`${theme === 'light' ? 'bg-zinc-100/70' : 'bg-zinc-900/70'} border ${theme === 'light' ? 'border-zinc-200/60' : 'border-zinc-700/40'} rounded-lg flex items-center ${isPortrait ? 'px-2 py-1 gap-1.5' : 'px-3 py-1.5 gap-2'
                          }`}
                      >
                ...
                >
                  {entity.emirate && (
                    <div className={isPortrait ? 'mb-2' : 'mb-3'}>
                      <h4 className={`${labelColor} text-xs font-light uppercase tracking-wider ${isPortrait ? 'mb-0.5' : 'mb-1'
                        }`}>
                        Headquarters
                      </h4>
                  ...
                  {entity.narration && (
                    <div>
                      ...
                      <p className={`${theme === 'light' ? 'text-zinc-700' : 'text-zinc-300'} font-light leading-relaxed ${isPortrait ? 'text-xs' : 'text-sm'
                        }`}>
                        {entity.narration}
                      </p>
                    </div>
                ...
                >
                  ...
                  <div className={`w-full rounded-lg overflow-hidden relative ${theme === 'light' ? 'bg-zinc-100/50' : 'bg-zinc-900/50'} flex items-center justify-center ${isPortrait ? 'h-32' : 'h-40'
                    }`}>
                    {/* Company image */}
                    {!imageError ? (
                      <Image
